...
lmrobust<-function(y,X,w, cmat=0){
numofcoef  = ncol(X)
...
o=conRLM_fit(X,y,w,Amat=cmat,method="M",wt.method="case",bvec=as.matrix(rep(0,nrow(cmat))),scale.est = c("MAD"),meq=nrow(cmat),psi=psi.huber)$coef
...
o=rlm(X, y, weights=w, init = "ls", psi = psi.huber, scale.est = c("MAD"), k2 = 1.345,  method = c("M"),wt.method = c("case"),  maxit = 200, acc = 1e-4, test.vec = "resid", lqs.control = NULL)
...
conRLM_fit <- function (x, y, weights, ..., w = rep(1, nrow(x)), init = "ls", 
                        ...
                        maxit = 200, acc = 1e-04, test.vec = "resid", 
